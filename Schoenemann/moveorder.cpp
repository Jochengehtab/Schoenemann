...
void orderMoves(Movelist& moveList, Hash* entry, Board& board, int scores[], Move killer, int ply)
{
	...
	for (int i = 0; i < moveList.size(); i++)
	{
		...
		{
			scores[i] += searcher.quietHistory[board.sideToMove()][board.at(move.from()).type()][move.to().index()] + 2 * searcher.stack[ply - 1].continuationHistory[searcher.stack[ply - 1].previousMovedPiece][searcher.stack[ply - 1].previousMove.to().index()][board.at(move.from()).type()][move.to().index()];
		}
	}
